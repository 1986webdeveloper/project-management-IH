...
const ProjectService = ({ dispatch, setLoading }: ProjectServiceProps) => {
	const CreateProject = async ({ payload, setOpen }: createProjectProps) => {
		setLoading(true);
		await axios(RequestHelper('POST', API_LIST.CREATE_PROJECT, { payload: payload }))
	...
	const UpdateProject = async ({ payload, setOpen, setIsEdit }: updateProjectProps) => {
		setLoading(true);
		await axios(
	...
	const DeleteProject = async ({ projectId }: DeleteProjectProps) => {
		setLoading(true);
		await axios(RequestHelper('DELETE', API_LIST.DELETE_PROJECT + `${projectId}`))
			.then(async (response: any) => {
				const _data = response.data;
				await GetProject();
				setLoading(false);
