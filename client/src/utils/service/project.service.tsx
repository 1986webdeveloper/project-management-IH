...
const ProjectService = () => {
	const CreateProject = ({ payload, setOpen, dispatch, projectList, setLoading }: createProjectProps) => {
		setLoading(true);
	...
	const UpdateProject = ({ payload, setOpen, setIsEdit, projectList, dispatch, setLoading }: updateProjectProps) => {
		setLoading(true);
		axios(
			...
			.then((response: any) => {
				...
				const index = allProjects.findIndex(obj => obj._id === updatedUser._id);
				if (index !== -1) {
	...
	const DeleteProject = ({ projectId, setLoading, dispatch, projectList }: DeleteProjectProps) => {
		setLoading(true);
