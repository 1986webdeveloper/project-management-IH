...
export function ImageUpload({ setError, setImgURL, imgURL }: imageUploadProps) {
  ...
  const onChange = async (
    ...
    // console.log(imageList, "imageListnnnn");
    ...
    const uploadImageToApi = async (imageData: any) => {
      try {
        ...
        if (response.data.data.imageUrl) {
          // console.log(response.data.data.imageUrl, "URL From Upoad Response");
  ...
  // console.log(imgURL ? `${BASEURL}/${imgURL}` : DEFAULT_PIC_URL, "000000");
