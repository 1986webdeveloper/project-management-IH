...
export function ImageUpload({ setError, setImgURL, imgURL }: imageUploadProps) {
	...
	const deleteImage = async () => {
		try {
			...
			if (response?.data?.response?.message || response?.data?.response?.message?.length) {
				setImages([]);
				setImgURL('');
			}
		} catch (error) {
					...
					<div className={styles.uploadWrapper}>
						{
							<div className={styles.imageWrapper}>
								...
								<div className={styles.buttonsWrapper}>
									<button onClick={() => onImageUpdate(0)} className={styles.button}>
										<RxUpdate size={22} />
									</button>
									{imgURL && (
										<button onClick={() => deleteImage()} className={styles.button} disabled={!imgURL ? true : false}>
